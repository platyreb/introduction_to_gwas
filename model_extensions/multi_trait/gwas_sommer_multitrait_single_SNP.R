...
thin = if(exists(x = "thin")) thin else 100
...
run_mt_gwas <- function(snp_name){
  ...
  # keep SNP rows
  snp_rows <- grep("SNP", beta_table$Effect)
...
results <- results |> inner_join(SNP_INFO, by = c("SNP" = "snp"))

## TRAIT 1
temp <- filter(results, trait == "SL")
